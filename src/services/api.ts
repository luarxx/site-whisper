...
export class ApiClient {
   ...
   * A configuração (URL, API key) já está no backend.
   */
  async createWhatsAppInstance(): Promise<WhatsAppInstanceData> {
    const { data } = await this.instance.post<WhatsAppInstanceData>('/whatsapp/instance');
    return data;
  }
